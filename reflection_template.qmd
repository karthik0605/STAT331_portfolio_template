...
[**Teacher Feedback Which Led To Revisions In This Portfolio**]{.underline}

**Lab 3 Question 5: It looks like you are doing the same function across multiple columns...I'd recommend a more efficient tool!\
\
Should teacher_id be treated as a numerical variable? These are a proxy for a teacher’s name. If the variable was a teacher’s name then it would be read in as a character...\
\
Should course_id be treated as a numerical variable? These are a proxy for a course subject and number. If the variable was recorded as STAT 331 then it would be read in as a character...**

**Lab 4 Question 7:** **I would encourage you to use named arguments, so your code is easier to understand!**

**Lab 4 Question 5: Can you give your column names better titles, so the reader knows what "2008" and "2018" represent? What are the values stored in these columns?\
\
...
**Challenge 7 Question 2: It looks like you are checking multiple conditions in your if_else() function. This is exactly what the case_when() function is built for!**

**Lab 8 Question 1: This code works but doesn’t use the most current code formatting for across()! Could you used named arguments? Could you incorporate the \~ and .x into the rescale_01() function?\
\
...
# Lab 3 Question 5 (Modified to use across instead of repeated functions, used different types)
...
# Modified Lab 4 Question 7 to use named arguments
ca_childcare |>
  rename(
      Infant = mc_infant,
      Toddler = mc_toddler,
...
# Modified Lab 5 Part 3 to use inner join
get_fit_now_member |>
...
# Modified Lab 5 Part 5 to use anti-joins for suspects who weren't interviewed to find the final killer
person |>
...
style_plot <- function(p,
  ...
  if (aesthetic == "fill") {
    p <- p + scale_fill_manual(values = palette, na.translate = FALSE)
  } else {
    p <- p + scale_color_manual(values = palette, na.translate = FALSE)
    #https://stackoverflow.com/questions/74695984/drop-na-values-from-scale-fill-either-in-aesthetics-or-in-the-scale
  }
...
# Modified Lab 4 Question 5 to have better column names w/ context
ca_childcare |>
  filter(study_year %in% c(2008, 2018)) |>
  mutate(mhi_2018_k = mhi_2018 / 1000) |>
  group_by(region, study_year) |>
...
![](images/clipboard-3992374880.png)
